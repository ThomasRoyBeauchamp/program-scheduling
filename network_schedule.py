...
class NetworkSchedule:
    ...
    def __init__(self, dataset_id, n_sessions, sessions=None, start_times=None, filename=None, save=True):
        """
    ...
    def generate_random_network_schedule(self, seed=42):
        ...
        self.sessions = []
        self.start_times = []
        for start_time in range(0, self.length, self.QC_LENGTH):
    ...
    def _calculate_probabilities(self):
        ...
        for k, v in self.dataset.items():
            t_sep = min_sep[k.split("/")[-1]]
            rate = self.length / (self.QC_LENGTH + t_sep)
            n_slots = sum([1 for b in SessionMetadata(k + "_alice.yml", session_id=1).blocks if b.type == "QC"]) * v
            probability = 1 / rate * n_slots
            p.update({k.split("/")[-1]: probability})
        return p
